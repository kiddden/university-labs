...
#     while True:
#         for i in range(k):
#             distances[:, i] = np.apply_along_axis(distance_func, 1, data.values, centroids[i])
...
#         for i in range(k):
#             centroids[i] = np.mean(data.values[cluster_assignments == i], axis=0)
...
# cluster_assignments, centroids = k_means_clustering(data.iloc[:, :-1], 3, euclidean_distance)

# print(cluster_assignments)

# c = Counter(cluster_assignments)
